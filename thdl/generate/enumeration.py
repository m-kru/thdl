...
class Enumeration():
    ...
    def generate_pkg(self):
        string = "function to_slv (e : " + self.identifier + ") return std_logic_vector;\n\n"
        ...
        if self.identifier.startswith("t_") or self.identifier.startswith("T_"):
            ...
            tmp = self.identifier
        name = "to_" + tmp
        string += "function " + name + " (slv : std_logic_vector(" + str(self.length - 1) + " downto 0)) return " + self.identifier + ";\n"

        return string

    def generate_pkg_body(self):
        string = "function to_slv (e : " + self.identifier + ") return std_logic_vector is\n"
        string += "begin\n"
        string += "   case e is\n"
        for i, v in enumerate(self.values):
            string += "      when " + v + " => return \"" + bin(i)[2:].zfill(self.length) + "\";\n"
        string += "   end case;\n"
        string += "end function;\n\n"
        ...
        if self.identifier.startswith("t_") or self.identifier.startswith("T_"):
            ...
            tmp = self.identifier
        name = "to_" + tmp
        string += "function " + name + " (slv : std_logic_vector(" + str(self.length - 1) + " downto 0)) return " + self.identifier + " is\n"
        string += "begin\n"
        string += "   case slv is\n"
        for i, v in enumerate(self.values):
            string += "      when \"" + bin(i)[2:].zfill(self.length) + "\" => return " + v + ";\n"
